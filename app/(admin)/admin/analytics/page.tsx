...
export default function AdminAnalyticsPage() {
  ...
  const handleCopySetupPrompt = () => {
    const promptLines = [
      ...
      '',
      "You are integrating this project into WarishLabs' centralized multi-tenant visitor analytics service.",
      'Do NOT create a separate database or schema — all telemetry events are stored centrally in Neon DB via warishlabs.in.',
      '',
      '---',
      '',
      '## 0. Technical Architecture & Ingest Payload Specs',
      '',
      '- **Ingest API Endpoint**: `https://warishlabs.in/api/analytics/event`',
      '- **Project Identifier**: Replace `<THIS_PROJECT_SLUG>` with your project slug (e.g. `toolkit`, `forgeflow`, `my-new-app`).',
      '- **CORS & Subdomains**: Cross-origin requests from ecosystem subdomains (`*.warishlabs.in`) are enabled on warishlabs.in.',
      '- **Fail-Safe Policy**: Visitor tracking MUST fail silently (`.catch(() => null)`). Network failures, ad blockers, or timeouts must NEVER throw errors or degrade user experience.',
      '',
      '### Required Ingest JSON Payload (POST request):',
      '```json',
      '{',
      '  "projectSlug": "<THIS_PROJECT_SLUG>",',
      '  "projectId": "<THIS_PROJECT_SLUG>",',
      '  "eventName": "page_view",',
      '  "url": "https://<THIS_PROJECT_DOMAIN>/path",',
      '  "path": "/path",',
      '  "referrer": null,',
      '  "visitorId": "v_unique_uuid_token"',
      '}',
      '```',
      '*(Note: Transmit both `projectSlug` and `projectId`. Referrer MUST be `null` if empty).*',
      '',
      '---',
      '',
      '## 1. What Antigravity MUST Do Automatically in Code',
      '',
      '1. **Framework & Layout Inspection**:',
      "   Identify the application structure (Next.js App Router/Pages Router, Vite, React, Svelte, or Vanilla JS) and locate the root layout/App component.",
      '',
      '2. **Environment Configuration**:',
      '   Update `.env.example` and local `.env` with:',
      '   `NEXT_PUBLIC_ANALYTICS_API_URL="https://warishlabs.in/api/analytics/event"`',
      '   `NEXT_PUBLIC_ANALYTICS_PROJECT_ID="<THIS_PROJECT_SLUG>"`',
      '',
      '3. **Create Fail-Safe Client Tracker Module (`src/lib/analytics.ts` or `lib/analytics.ts`)**:',
      '   Implement first-party visitor ID generation (`warishlabs_vid` stored in `localStorage`) and event tracking dispatch:',
      '   ```typescript',
      ...
      '   Mount an AnalyticsTracker component or route listener in the root layout so initial page loads and SPA route changes dispatch `trackPageView()`.',
      '',
      '5. **Build & Typecheck Verification**:',
      '   Run `npm run typecheck` (or `tsc --noEmit`) and `npm run build` to guarantee clean compilation without typescript or build errors.',
      '',
      '---',
      '',
      '## 2. What the USER MUST Do Manually',
      '',
      'Explain to the user clearly what they must do outside of code:',
      '1. **Register Project in WarishLabs Admin Console**:',
      '   - Open `https://warishlabs.in/admin/analytics`.',
      '   - Click **+ Add Project**.',
      '   - Fill in Project Name (e.g. `ForgeFlow`), Project Slug (`forgeflow`), and Domain (`forgeflow.warishlabs.in`).',
      '   - *(Hits auto-register, but adding it gives it a friendly display name in the dashboard filter dropdown).*',
      '2. **Set Vercel Environment Variables**:',
      '   - Open Vercel Project Settings for this project.',
      '   - Go to **Environment Variables** and add:',
      '     - `NEXT_PUBLIC_ANALYTICS_API_URL` = `https://warishlabs.in/api/analytics/event`',
      '     - `NEXT_PUBLIC_ANALYTICS_PROJECT_ID` = `<your-project-slug>`',
      '   - Apply to Production, Preview, and Development environments.',
      '3. **Redeploy Project on Vercel**:',
      '   - Trigger a new deployment so the production build includes the environment variables.',
      '',
      '---',
      '',
      '## 3. Final Summary Output Requirement',
      '',
      'At the very end of your response, output two bulleted sections:',
      '',
      '### What Antigravity Completed Automatically:',
      '- Created `lib/analytics.ts` fail-safe client tracker.',
      '- Integrated global route listener in root layout.',
      '- Updated `.env.example` with analytics variables.',
      '- Verified build and typecheck suites pass.',
      '',
      '### What You Need to Do Manually:',
      '- Add project under `https://warishlabs.in/admin/analytics` via **+ Add Project**.',
      '- Set `NEXT_PUBLIC_ANALYTICS_API_URL` and `NEXT_PUBLIC_ANALYTICS_PROJECT_ID` in Vercel settings.',
      '- Trigger a Vercel redeployment.',
    ];

    navigator.clipboard.writeText(promptLines.join('\n'));
    setCopied(true);
    toast.success('Enhanced setup prompt copied to clipboard!');
