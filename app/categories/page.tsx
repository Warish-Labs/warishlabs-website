            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {categories.map((cat) => (
                <Link key={cat.id} href={`/categories/${cat.slug}`} className="block h-full">
                  <Card className="glass-panel border border-white/10 bg-white/5 backdrop-blur-sm p-6 h-full flex flex-col justify-between hover:border-accent/40 hover:scale-[1.02] transition-all duration-300 cursor-pointer">
                    <div className="space-y-3">
                      ...
                      <CardTitle className="text-xl font-black tracking-tight text-white pt-2">
                        {cat.name}
                      </CardTitle>
                    ...
                    <div className="pt-6 flex justify-end text-[10px] uppercase font-bold tracking-widest text-accent">
                      Open Landscape <ArrowRight className="w-3.5 h-3.5 ml-1.5 inline" />
                    </div>
                  </Card>
                </Link>
              ))}
