...
export class EmailService {
  ...
  static async sendContactEmail(name: string, email: string, subject: string, message: string): Promise<boolean> {
    ...
    try {
      ...
      AlertService.sendRuntimeAlert('Resend', 'sendContactEmail', error);
      return false;
    }
  }
  ...
  static async sendNewsletterWelcome(email: string, unsubscribeToken: string): Promise<boolean> {
    const unsubscribeUrl = `${process.env.NEXT_PUBLIC_APP_URL || 'https://warishlabs.in'}/api/newsletter/unsubscribe?token=${unsubscribeToken}`;
    ...
    const htmlContent = `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #eee; border-radius: 8px;">
        ...
        <p>You'll now receive updates about our newest products, engineering case studies, and software releases.</p>
        <p>Stay curious,</p>
    ...
    try {
      ...
      AlertService.sendRuntimeAlert('Resend', 'sendNewsletterWelcome', error);
      return false;
    }
  }

  /**
   * Dispatches a broadcast campaign email in batches with 10-second throttled delays
   */
  static async sendBroadcastCampaign(
    ...
    // Send in batches of 10 with a 10-second gap between batches to avoid spam bursts
    const batchSize = 10;
    for (let i = 0; i < emails.length; i += batchSize) {
      if (i > 0) {
        // Throttling: 10 second delay between broadcast batches
      ...
      try {
        ...
        AlertService.sendRuntimeAlert('Resend', 'sendBroadcastCampaign', err);
        failureCount += batchEmails.length;
      }
    }
  ...
  static async sendMessageReply(
    ...
    try {
      ...
      AlertService.sendRuntimeAlert('Resend', 'sendMessageReply', error);
      return false;
    }
  }
}
